...
function draw() {
  ...
  if (!sort) {
    ...
    for (let i = 0; i < 500; i++) {
      ...
      swapPix(); //  check right neighbor and switch with with one below
    }
...
function swapPix() {
  ...
  const x = floor(random(1, img.width / 2)); // so not out of range
  // use the same height
  const y = floor(random(img.height - 1));

  const x1 = floor(random(img.width / 2, img.width)); // so not out of range
  //const y1 = floor(random(img.height - 1));
  const y1 = y;
  // Get the position in the pixel array
  let pos = 4 * (y * img.width + x);
  let pos1 = 4 * (y1 * img.width + x1);
  //let pos2 = 4*((y+1)*img.width+(x))

  //switch those values

  // make a copy for switching
  let temp = [];
  ...
  img.pixels[pos] = img.pixels[pos1];
  img.pixels[pos + 1] = img.pixels[pos1 + 1];
  img.pixels[pos + 2] = img.pixels[pos1 + 2];
  //img.set(x, y + 1, colorOne);
  img.pixels[pos1] = temp[0];
  img.pixels[pos1 + 1] = temp[1];
  img.pixels[pos1 + 2] = temp[2];
}
